...
function loadFrameScript(tab, script) {
  let mm = tab._browser.messageManager;
  let once = !!script.name;
  if (typeof script === 'function') {
    script = (once ? 'let ' + script.name + ' = ' + script.name + ' || new ' : 'new ') + script;
  }
  mm.loadFrameScript('data:,' + script, true);
}

function addMessageListener(tab, name, listener) {
  let mm = tab._browser.messageManager;
...
const TabTrait = Trait.compose(EventEmitter, {
  ...
  constructor: function Tab(options) {
    ...
    let tt = this;
    console.log('KKKKKKKKKKKKKKKKKK', this);
    addMessageListener(this, 'sdk.tab.event', function({ data, objects }) {
      console.log('EEEEEEEEEEEEEEEEEEEEEEVENTd'+data.type, data, objects.event);
      if (data.type === EVENTS.ready.dom) {
        // console.log('RRRRRR', data.type, objects.event);
        tt._onReady(objects.event);
      }
      if (data.type === EVENTS.load.dom) {
        // console.log('LLLLLL', data.type, objects.event);
        tt._onLoad(objects.event);
      }
      if (data.type === EVENTS.pageshow.dom) {
        // console.log('SSSSSSSSSSS', data.type, objects.event);
        tt._onPageShow(objects.event);
      }
    })

    loadFrameScript(this, function sdk_tab_events() {
      sendAsyncMessage('sdk.tab.event', { type: 'log' }, { event: content.location.href });
      ['DOMContentLoaded', 'load', 'pageshow'].forEach( type => {
        docShell.chromeEventHandler.addEventListener(type, (e) => {
            sendAsyncMessage('sdk.tab.event', { type: type }, { event: e });
        }, true);
      })
    })
  ...
  getThumbnail: function getThumbnail()
    this._tab ? 11 || getThumbnailURIForWindow(this._contentWindow) : undefined,
  /**
